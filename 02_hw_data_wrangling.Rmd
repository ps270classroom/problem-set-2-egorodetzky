...
party_incumbent_promote <- fb_ads |>
  ...
  summarize(promote_prop = mean(ad_tone_promote, na.rm = TRUE), .groups = 'drop')
party_incumbent_promote
knitr::kable(party_incumbent_promote, col.names = c("Party", "Incumbency", "Promote Prop"), digits = 3)
```
...
plot_q3 <- ggplot(data = fb_ads, aes(x = impressions_millions)) +
  geom_histogram()
plot_q3
```

The shape of the histogram is one column almost at 600 and the rest lower. Most of the House candidates did not have more than 10 million Facebook ad impressions.
...
top_cand <- fb_ads |>
  ...
  slice_head(n = 15) |>
  mutate(cand_name_last = fct_reorder(cand_name_last, impressions_millions))

fb_top_plot <- ggplot(data = top_cand, mapping = aes(x = impressions_millions, y = cand_name_last, fill = party)) + 
  geom_bar(stat = "identity") +
  labs(title = "Top 15 House Candidate Facebook Ad Impressions, 2017-2018", x = "Facebook Ad Impressions in Millions", y = "Candidates") +
